...
public class PickupSpawner : MonoBehaviour
{
        ...
        for (int i = 0; i < pickupCount; i++)
        {
            ...
            foreach (PickupSpawnSettings pickup in pickupSettings)
            {
                if (randRoll < currWeightVal + pickup.spawnWeight)
                {
                    ...
                    GameObject pkObj = Instantiate(pickup.spawnPrefab, spawnPoint, Quaternion.identity);
                    SceneManager.MoveGameObjectToScene(pkObj, SceneManager.GetSceneByName("N_GravityTest"));
                    break;
                }
    ...
    void SpawnDropOffZone()
    {
        ...
        for (int i = 0; i < dropOffZoneCount; i++)
        {
            ...
            SceneManager.MoveGameObjectToScene(dropoff.gameObject, SceneManager.GetSceneByName("N_GravityTest"));
